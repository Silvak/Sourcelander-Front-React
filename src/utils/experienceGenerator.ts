...
const EXPERIENCE_LEVELS: IExperienceLevelData[] = [
  {
    ...
    maxYears: 2,
    description:
      "Desarrollador con poca experiencia, ideal para proyectos simples",
  },
  {
    level: "Mid-level",
    minYears: 3,
    maxYears: 5,
    description:
      "Desarrollador con experiencia intermedia, puede manejar proyectos complejos",
  },
  {
    level: "Senior",
    minYears: 6,
    maxYears: 10,
    description:
      "Desarrollador experimentado, líder técnico en proyectos complejos",
  },
  {
    level: "Expert",
    minYears: 11,
...
export function generateProfessionalExperience(
  ...
  for (let i = 0; i < count && remainingYears > 0; i++) {
    ...
    const endYear =
      i === 0 ? currentYear : currentYear - (experienceYears - remainingYears);
...
export function getExperienceLevel(years: number): IExperienceLevelData {
  return (
    EXPERIENCE_LEVELS.find(
      (level) => years >= level.minYears && years <= level.maxYears
...
function getPrimaryPosition(
  skills: string[],
  experienceLevel: IExperienceLevelData
): string {
  const positions =
    POSITIONS_BY_LEVEL[
      experienceLevel.level as keyof typeof POSITIONS_BY_LEVEL
    ] || POSITIONS_BY_LEVEL["Mid-level"];
  ...
  if (skills.length > 0) {
    ...
    if (
      skillLower.some((s) =>
        ["react", "vue", "angular", "frontend"].includes(s)
      )
    ) {
      return (
        positions.find((p) => p.toLowerCase().includes("frontend")) ||
        positions[0]
      );
    }

    if (
      skillLower.some((s) => ["node", "python", "java", "backend"].includes(s))
    ) {
      return (
        positions.find((p) => p.toLowerCase().includes("backend")) ||
        positions[0]
      );
    }
    ...
    if (
      skillLower.some((s) =>
        ["react native", "ios", "android", "mobile"].includes(s)
      )
    ) {
      return (
        positions.find((p) => p.toLowerCase().includes("mobile")) ||
        positions[0]
      );
    }
    ...
    if (
      skillLower.some((s) =>
        ["devops", "aws", "docker", "kubernetes"].includes(s)
      )
    ) {
      return (
        positions.find((p) => p.toLowerCase().includes("devops")) ||
        positions[0]
      );
    }
  }
...
export function generateFreelancerExperience(freelancerData: {
  ...
  const experienceCount = Math.min(
    Math.max(2, Math.floor(experienceYears / 2)),
    5
  );
 ...
 */
export function validateProfessionalExperience(
